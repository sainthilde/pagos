package pe.gob.vuce.cp2.bs.infrastructure.out.postgresql.entity;

import java.io.Serializable;

import jakarta.persistence.Column;
import jakarta.persistence.Entity;
import jakarta.persistence.GeneratedValue;
import jakarta.persistence.GenerationType;
import jakarta.persistence.Id;
import jakarta.persistence.Table;
import lombok.AllArgsConstructor;
import lombok.Builder;
import lombok.Getter;
import lombok.NoArgsConstructor;
import lombok.Setter;


@Entity
@Table(name = "motivo_escala", schema = "\"ESCALA\"")
@Getter
@Setter
@NoArgsConstructor
@AllArgsConstructor
@Builder
public class MotivoEscalaEntity implements Serializable{

    @Id
    @GeneratedValue(strategy = GenerationType.IDENTITY)
    @Column(name = "motivo_escala_id")
    private Integer motivoEscalaId;

    @Column(name = "escala_id")
    private Integer escalaId;

    @Column(name = "motivo_id")
    private Integer motivoId;

    @Column(name = "ind_principal")
    private Boolean indPrincipal;

    @Column(name = "estado", nullable = false, length = 1)
    private String estado;

  
}